...
from ui.Ui_schedule_student import Ui_schedule_student

class schedule_student(Ui_schedule_student):
    ...
    def getSno(self):
        ...
        if Sno_str :
            ...
            # 类型转换错误，弹窗提示用户
                createErrorInfoBar(self,"输入错误", "学号输入无效 请输入一个有效的12位整数。")
    ...
    def select_lesson(self):
        ...
        if sno_str:
            ...
            db.connect()
            # 先查询所属班级
            query = """ SELECT c.Cname,c.Cno
                        FROM sms_class c,sms_students s
                        WHERE
                        (
                            s.Sno = %s AND
                            s.Cno = c.Cno
            ...
            # 存在该学生
            if result:
                self.set_class_label(result)
                query = """ SELECT s.Sname,l.Lno,l.Lname,l.Lhours,l.Lsemester,l.Lcredits,l.Lexam,sh.Tno,t.Tname
                            FROM sms_schedule sh, sms_lesson l,sms_teacher t,sms_students s
                            WHERE
                            (
                                s.Sno = %s AND
                                s.Cno = sh.Cno AND
                ...
                if result:
                    ...
                    createWarningInfoBar(self,"提示", "该学生所在班级的课表为空",InfoBarPosition.TOP)
            else:
                createWarningInfoBar(self,"警告", "不存在该学生或其不属于任何班级",InfoBarPosition.TOP)
            db.disconnect()     
        else:
            createErrorInfoBar(self,"查看失败", "请先输入学号",InfoBarPosition.TOP)

    def set_class_label(self,data):
        self.Class_label.setText(data[0][0])
        self.Cno_label.setText(data[0][1])
    ...
    def fill_table(self,data):
        ...
        col_names=['姓名','课程编号',"课程名称",'学时','学期','学分','考试/考查','教师编号',"教师姓名"]
